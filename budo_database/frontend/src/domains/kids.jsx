    ...
    <FirstAidGallery entries={[...(kid.notes || []), ...(kid.first_aid_entries || [])]} childName={kid.full_name}>
      <Columns className="kid-detail-grid">
        <Column id="left-column">
          ...
          <Card title="BuDo" id="budo-container"><FieldList items={[["Turnus", data.turnus?.label], ["Budo Familie", kid.budo_family], ["Haus", kid.special_family], ["SWP 1", kid.focus_w1], ["SWP 2", kid.focus_w2]]} /><div className="react-actions"><Button href={`/${kid.present ? 'check_out' : 'check_in'}/${kid.id}`}>{kid.present ? 'Auschecken' : 'Einchecken'}</Button></div></Card>
        </Column>
        ...
        <Column id="right-column">
          ...
          <Card title={`Pfand: ${kid.deposit}`} id="pfand"><div className="react-actions deposit-actions"><Button type="button" onClick={() => deposit('increase')}>+ Pfand</Button><Button type="button" variant="secondary" onClick={() => deposit('decrease')}>− Pfand</Button></div></Card>
        </Column>
      </Columns>
