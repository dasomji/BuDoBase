...
def _require_product_admin(user):
    if not user.is_superuser:
        raise PermissionDenied("Admin team management access denied.")
...
def admin_teams_page(request):
    ...
    _require_product_admin(request.user)
...
def set_membership_leadership(request, membership_id):
    ...
    return Response({"membership_id": membership.pk, "functional_role": role})
